...
### 1.1 Stale `gerbera-core/tests/integration_tcp.rs` keeps breaking [S]
...
Slots into `spawn_worker` (gerbera-core/src/server.rs:474) — three new lines around the existing loop.
...
**Problem**: gerbera supports persistent done markers (`done_marker_path`), but the resume flow isn't documented as a feature. Users have to know to set the basedir, see that markers are written, and trust that re-running with the same parameters will skip done blocks. There's no "try the run; if it fails halfway, here's how to pick up where it left off" guide.

**Proposal**:

- A `gerbera.resume(...)` helper that re-reads the markers, prints a "resuming with N/M blocks already done" summary, and runs the rest.
...
**Next Steps**: This seems like a great feature to implement. It shouldn't really touch anything internally in gerbera and should just be its own standalone observer. Seems great.

### 2.5 Multi-machine workers [L, high value if you need it]

**Problem**: gerbera binds to `127.0.0.1` by default. Users running on multi-node SLURM/LSF clusters can't farm out workers across nodes; daisy has launchers for this (`daisy.distributed`).

**Proposal**:

- Bind option (`Server(host="0.0.0.0", port=...)` already supported in Rust; expose to Python).
- A worker entry point: `python -m gerbera.worker --host=... --port=... --task-id=...` that connects to a remote server and runs the task's `process_function`. The user's process function would need to be importable on each node.
- Optional SLURM helper: `gerbera.distributed.launch_workers(server_addr, n_per_node=...)` that submits sbatch jobs.
...
**Next Steps**: This is a high priority. I'm not sure quite how necessary it is at the moment since I haven't started using `gerbera` as a replacement for daisy, but this definitely seems like an issue that will have to be resolved before I start using it.
...
**Next Steps**: This should be implemented sooner rather than later. It seems like a simplification refactor that doesn't really change anything about how `gerbera` actually works. I think we can go ahead and do this now.
...
`BlockFailed` carries the error string already (gerbera-core/src/protocol.rs). Just need to keep the first N per task in `RunStats` and render them.
...
**Proposal**: `gerbera.run_blockwise(tasks, dry_run=True)` prints the dependency graph, per-task block count, ROI, and resource budget allocation, then returns. Useful for diagnosing "I expected 80 blocks but I'm seeing 64" (off-by-one in fit/conflict).
...
Things gerbera does well that we shouldn't accidentally walk back when refactoring:
...
1. **2.8 lazy roots** — fixes the 1M-block startup hang foot-gun; small follow-on to the dep-graph cleanup that already landed
2. **2.1 init/cleanup hooks** — biggest user-visible win for the smallest amount of code
3. **2.3 resume UX** — already-built feature, missing only docs and a helper
4. **2.9 failure summary** — small change, removes a real pain point

Avoid bundling 2.5 (multi-machine) with anything else — it's a separate design discussion and probably its own RFC.
