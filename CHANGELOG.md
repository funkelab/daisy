...
- Lint/type CI (`lint.yaml`): `ruff check` + `ruff format --check` with a
  ...
  (no Rust build needed in CI). `benchmarks/` is linted along with
  everything else.

### Fixed

- **The bundled benchmarks no longer compare daisy against itself.** The
  `gerbera -> daisy` rename left both arms of every comparison pointing at
  the same package: each script defined `bench_daisy` twice (the second
  definition shadowing the first, so one arm was silently discarded) and
  built result dicts with a duplicate `"daisy"` / `"daisy_s"` key. The
  committed artifacts encoded that — `dep_graph_results.json` shipped
  literal duplicate JSON object keys, and `block_scaling_results.json`
  reported a 28.44x "speedup" that was noise between two runs of the same
  code. There is no second implementation in this repository, so the
  vestigial daisy-1.x arm is gone:

  - `bench_dependency_graph.py` is now a single-implementation scaling
    benchmark (block count / conflict levels / context vs. enumeration
    throughput).
  - `bench_worker_scaling.py` compares the two `process_function`
    modalities that do exist — 1-arg block function vs. 0-arg worker
    function — using the distributed `Server` at every worker count
    (it previously used the in-process `SerialServer` for one arm at
    `workers=1` only, which is a different execution model), with
    `progress=False` so tqdm rendering stays out of the timed region, and
    asserting `completed_count == total_block_count` rather than
    `is_done()`, which only means the counters balance and is also true of
    a run in which every block failed.
