...
Compares three configurations:
    ...
    - gerbera block-fn     (Rust server + tokio TCP, Rust-driven worker loop)
    - gerbera worker-fn    (Rust server + tokio TCP, Python-driven worker loop
                            via `gerbera.Client`)
"""
...
# --- Gerbera block-function benchmark ---
...
def bench_gerbera(num_blocks, num_workers):
    def noop(block):
        pass
    ...
    task = gerbera.Task(
        "bench",
        total_roi=gerbera.Roi([0], [total_size]),
...
# --- Gerbera worker-function benchmark ---
...
# num_workers=1.
def bench_gerbera_worker_fn(num_blocks, num_workers):
    def worker():
    ...
    t0 = time.perf_counter()
    states = gerbera.Server().run_blockwise([task])
    elapsed = time.perf_counter() - t0
...
def run_scaling():
    ...
    header = (f"{'workers':>8} | {'daisy':>10} | {'gerbera':>10} | "
              f"{'gerbera-wf':>11} | {'speedup':>8}")
    print(header)
    print("-" * len(header))

    results = []
    for nw in worker_counts:
        g_time, g_done, g_count = bench_gerbera(num_blocks, nw)
        assert g_done, f"gerbera failed: {g_count}/{num_blocks}"

        w_time, w_done, w_count = bench_gerbera_worker_fn(num_blocks, nw)
        assert w_done, f"gerbera worker-fn failed: {w_count}/{num_blocks}"
...
def run_block_scaling():
    ...
    header = (f"{'blocks':>8} | {'daisy':>10} | {'gerbera':>10} | "
              f"{'gerbera-wf':>11} | {'speedup':>8}")
    print(header)
    print("-" * len(header))
    ...
    for nb in block_counts:
        g_time, g_done, _ = bench_gerbera(nb, num_workers)
        w_time, w_done, _ = bench_gerbera_worker_fn(nb, num_workers)
...
def plot_results(worker_results, block_results):
    ...
    DAISY_COLOR = "#4878CF"
    GERBERA_BLOCK_COLOR = "#D65F5F"
    GERBERA_WORKER_COLOR = "#956CB4"

    # --- Plot 1: Worker scaling (absolute time) ---
    ax = axes[0]
    workers = [r["workers"] for r in worker_results]
    d_times = [r["daisy_s"] for r in worker_results]
    g_times = [r["gerbera_s"] for r in worker_results]
    w_times = [r["gerbera_worker_s"] for r in worker_results]

    ax.plot(workers, d_times, 'o-', label="daisy", color=DAISY_COLOR, linewidth=2)
    ax.plot(workers, g_times, 's-', label="gerbera block-fn",
            color=GERBERA_BLOCK_COLOR, linewidth=2)
    ax.plot(workers, w_times, '^-', label="gerbera worker-fn",
            color=GERBERA_WORKER_COLOR, linewidth=2)
    ...
    # --- Plot 2: Speedup vs workers (both gerbera variants over daisy) ---
    ax = axes[1]
    g_speedups = [r["daisy_s"] / r["gerbera_s"] if r["gerbera_s"] > 0 else 0
                  for r in worker_results]
    w_speedups = [r["daisy_s"] / r["gerbera_worker_s"] if r["gerbera_worker_s"] > 0 else 0
                  for r in worker_results]
    x = range(len(workers))
    width = 0.4
    ax.bar([i - width / 2 for i in x], g_speedups, width,
           label="gerbera block-fn", color=GERBERA_BLOCK_COLOR, edgecolor="black")
    ax.bar([i + width / 2 for i in x], w_speedups, width,
           label="gerbera worker-fn", color=GERBERA_WORKER_COLOR, edgecolor="black")
    ax.set_xlabel("Number of workers")
    ax.set_ylabel("Speedup over daisy")
    ax.set_title("Gerbera Speedup vs Daisy")
    ax.set_xticks(x)
    ax.set_xticklabels(workers)
    ...
    g_times = [r["gerbera_s"] for r in block_results]
    w_times = [r["gerbera_worker_s"] for r in block_results]

    ax.plot(blocks, d_times, 'o-', label="daisy", color=DAISY_COLOR, linewidth=2)
    ax.plot(blocks, g_times, 's-', label="gerbera block-fn",
            color=GERBERA_BLOCK_COLOR, linewidth=2)
    ax.plot(blocks, w_times, '^-', label="gerbera worker-fn",
            color=GERBERA_WORKER_COLOR, linewidth=2)
