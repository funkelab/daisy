...
class BatchTask():
    ...
    def parse_args(ap):
        ...
        try:
            ...
            ap.add_argument(
                "--db_name", type=str, help='MongoDB database project name',
                default=None)
            ap.add_argument(
                "--overwrite", type=int, help='Whether to overwrite completed blocks',
                default=0)
            ap.add_argument(
                "--num_workers", type=int, help='Number of workers to run',
                default=4)
            ap.add_argument(
                "--no_launch_workers", type=int, help='Whether to run workers automatically',
                default=0)
            ap.add_argument(
                "--config_hash", type=str, help='config string, used to keep track of progress',
                default=None)
            ap.add_argument(
                "--task_name", type=str, help='Name of task, default to class name',
                default=None)
    ...
    def _write_config(self, worker_filename, extra_config=None):
        ...
        self.config_file = os.path.join(
            '.run_configs', '%s.config' % self.db_id)

        self.new_actor_cmd = 'python %s run_worker %s' % (
            worker_filename, self.config_file)
    ...
    def init_callback_fn(self, context):
        ...
        print("sbatch command: DAISY_CONTEXT={} sbatch --parsable {}\n".format(
                context.to_env(), self.sbatch_file))

        print("Terminal command: DAISY_CONTEXT={} {}\n".format(
