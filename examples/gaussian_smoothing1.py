...
if __name__ == '__main__':
    ...
    ap.add_argument("--out_ds_name", type=str, default=None,
                    help='The name of the dataset, defaults to be in_ds_name + smoothed')
    ap.add_argument('--sigma', '-s', type=float,
                    help="Sigma to use for gaussian filter",
                    default=2)
    ap.add_argument('--block_read_size', '-r', nargs='+',
                    help="Size of block read region",
                    default=[20, 200, 200])
    ap.add_argument('--block_write_size', '-w', nargs='+',
                    help="Size of block write region",
                    default=[18, 180, 180])
    ap.add_argument("--num_workers", type=int, help='Number of workers to run',
                    default=4)
    ...
    assert len(config.block_read_size) == ndims,\
        "Read size must have same dimensions as in_file"
    assert len(config.block_write_size) == ndims,\
